...
import { signup, loginWithGoogle, useAuth } from '../../lib/useAuth';
...
const SignupPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validatePassword(password)) {
      setError('Password must be 6-12 characters and include at least one symbol.');
      return;
    }
    ...
    try {
      ...
      if (user) {
        await setDoc(doc(db, 'users', user.uid), { plan: 'free' }, { merge: true });
      }
    ...
    try {
      ...
      const unsubscribe = auth.onAuthStateChanged(async (user) => {
        if (user) {
          await setDoc(doc(db, 'users', user.uid), { plan: 'free' }, { merge: true });
          unsubscribe();
          toast.success('Signed up! Please choose a plan to continue.');
          router.push('/pricing');
        }
      });
    } catch (err: any) {
